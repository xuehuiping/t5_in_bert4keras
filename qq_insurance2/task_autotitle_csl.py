...
train_data = load_data('urls2_result.tsv')
...
class Evaluator(keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        if metrics['bleu'] > self.best_bleu:
            ...
            model.save_weights('./best_model_{}.weights'.format(epochs))  # 保存模型
...
def main7():
    '''加载权重，使用文件进行预测'''
    model.load_weights('./best_model_{}.weights'.format(epochs))
